...
let BufferFrom = require("buffer-from");
...
function Transaction(options) {
  ...
  (this.public_key = options.public_key),
    ...
    (this.getBearerToken = function () {
      ...
      public_key.importKey(BufferFrom(certificate), "public");

      // Encryt API key (data) using public key
      let token = public_key.encrypt(BufferFrom(this.api_key));

      // Return formatted string, Bearer token in base64 format
      return "Bearer " + BufferFrom(token).toString("base64");
    });
  ...
  (this.c2b = async function (transaction_data) {
    let request = {
      method: "post",
      url:
        "https://" + this.api_host + ":18352/ipg/v1x/c2bPayment/singleStage/",
      data: {
